...
(async () => {
  ...
  result = await request('/api/opportunity-workspaces', 'tenant-a', { method: 'POST', body: JSON.stringify({ title: 'Boundary qualification', capability_profile: { service_capabilities: ['conversion'], delivery_constraints: [], geography: ['UK'], capacity: 'one project', exclusions: [], disqualifiers: [] } }) });
  const boundaryWorkspaceId = result.body.workspace_id;
  for (const lead_id of ['A','B','C']) await request(`/api/opportunity-workspaces/${boundaryWorkspaceId}/candidates`, 'tenant-a', { method: 'POST', body: JSON.stringify({ lead_id, comparison_context: 'BOUNDARY_CHECK', evidence_window: '2026-07-19' }) });
  result = await request(`/api/opportunity-workspaces/${boundaryWorkspaceId}/evaluations`, 'tenant-a', { method: 'POST', body: JSON.stringify({ expected_version: 0 }) });
  assert.strictEqual(result.response.status, 200); assert.strictEqual(result.body.result, 'NO_WINNER'); assert(result.body.outcomes.every(item => item.decisive_reason.includes('Geography is not evidenced')));
  ...
  const selectedSnapshotId = result.body.lead_snapshot_id;
  const alternativeSnapshotId = result.body.outcomes.find(item => item.candidate_snapshot_id !== selectedSnapshotId).candidate_snapshot_id;
  result = await request(`/api/opportunity-workspaces/${workspaceId}/offer`, 'tenant-a', { method: 'POST', body: '{}' });
  assert.strictEqual(result.response.status, 409); assert.strictEqual(result.body.code, 'SELECTION_ACCEPTANCE_REQUIRED');
  ...
  result = await request(`/api/opportunity-workspaces/${workspaceId}/offer`, 'tenant-a', { method: 'POST', body: '{}' });
  assert.strictEqual(result.response.status, 409); assert.strictEqual(result.body.code, 'SELECTION_ACCEPTANCE_REQUIRED');
  result = await request(`/api/opportunity-workspaces/${workspaceId}/selection-decision`, 'tenant-a', { method: 'POST', body: JSON.stringify({ decision: 'ACCEPTED', selected_candidate_snapshot_id: selectedSnapshotId, rationale: 'Attempt unchanged progression.' }) });
  assert.strictEqual(result.response.status, 409); assert.strictEqual(result.body.code, 'CHALLENGE_UNRESOLVED');
  result = await request(`/api/opportunity-workspaces/${workspaceId}/selection-decision`, 'tenant-a', { method: 'POST', body: JSON.stringify({ decision: 'ACCEPTED', selected_candidate_snapshot_id: alternativeSnapshotId, rationale: 'Customer chooses an evidence-backed alternative.' }) });
  assert.strictEqual(result.response.status, 200); assert.strictEqual(result.body.decision, 'ACCEPTED');
  result = await request(`/api/opportunity-workspaces/${workspaceId}/offer`, 'tenant-a', { method: 'POST', body: '{}' });
  ...
  assert.strictEqual(result.response.status, 200); assert.strictEqual(result.body.decision_nodes.length, 24); assert.strictEqual(result.body.actions[0].state, 'COMPLETED');
